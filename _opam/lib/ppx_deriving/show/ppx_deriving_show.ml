    ...
    | { ptyp_desc = Ptyp_variant (fields, _, _); ptyp_loc } ->
      let cases =
        fields |> List.map (fun field ->
          match field with
          | 
# 176 "ppx_deriving_show.cppo.ml"
             {prf_desc = Rtag(label,  true (*empty*),  []); _}  
# 176 "ppx_deriving_show.cppo.ml"
                                                 ->
            
# 178 "ppx_deriving_show.cppo.ml"
            let label = label.txt in
            
# 180 "ppx_deriving_show.cppo.ml"
...
# 184 "ppx_deriving_show.cppo.ml"
            let label = label.txt in
            
# 186 "ppx_deriving_show.cppo.ml"
            Exp.case (Pat.variant label (Some [%pat? x]))
                     [%expr Ppx_deriving_runtime.Format.fprintf fmt [%e str ("`" ^ label ^ " (@[<hov>")];
                            [%e expr_of_typ typ] x;
                            Ppx_deriving_runtime.Format.fprintf fmt "@])"]
          | 
# 190 "ppx_deriving_show.cppo.ml"
