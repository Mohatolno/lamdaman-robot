...
# 38 "ppx_deriving.cppo.ml"
type tyvar = string Location.loc

# 43 "ppx_deriving.cppo.ml"
type deriver = {
  name : string ;
...
# 158 "ppx_deriving.cppo.ml"
  ...
  let get_attr ~deriver conv attr =
    ...
    | None -> None
    | Some (
# 196 "ppx_deriving.cppo.ml"
             { attr_name = 
                                               { txt =  name; loc = loc }; 
                                              attr_payload = 
                           PStr [{ pstr_desc = Pstr_eval (expr, []) }]; 
                                              attr_loc = _ } 
# 197 "ppx_deriving.cppo.ml"
                                                                       ) ->
...
# 222 "ppx_deriving.cppo.ml"
   { attr_name = 
                                                { txt =  "ocaml.warning"; loc = loc }; 
                                              attr_payload =  PStr [structure]; 
                                              attr_loc = loc } 

# 224 "ppx_deriving.cppo.ml"
type quoter = {
  mutable next_id : int;
...
let attr ~deriver name attrs =
  ...
  in
  let attr_starts prefix (
# 292 "ppx_deriving.cppo.ml"
                           { attr_name = 
                                               { txt =  txt; loc = _loc }; 
                                              attr_payload =  _; 
                                              attr_loc = _ } 
# 292 "ppx_deriving.cppo.ml"
                                                      ) = starts prefix txt in
  let attr_is name (
# 293 "ppx_deriving.cppo.ml"
                     { attr_name = 
                                               { txt =  txt; loc = _loc }; 
                                              attr_payload =  _; 
                                              attr_loc = _ } 
...
# 342 "ppx_deriving.cppo.ml"
        let name = mkloc name param.ptyp_loc in
        
# 344 "ppx_deriving.cppo.ml"
        fn accum name
      | _ -> assert false)
    accum params
...
# 360 "ppx_deriving.cppo.ml"
        let name = mkloc name param.ptyp_loc in
        
# 362 "ppx_deriving.cppo.ml"
        fn name accum
      | _ -> assert false)
    params accum
...
# 378 "ppx_deriving.cppo.ml"
      [mkloc name typ.ptyp_loc]
    
# 382 "ppx_deriving.cppo.ml"
...
# 387 "ppx_deriving.cppo.ml"
      [mkloc name typ.ptyp_loc]
      
# 391 "ppx_deriving.cppo.ml"
      @ free_in x
    ...
    | { ptyp_desc = Ptyp_variant (rows, _, _) } ->
      List.map (
          function 
# 396 "ppx_deriving.cppo.ml"
                    {prf_desc = Rtag(_, _, ts); _}  
# 396 "ppx_deriving.cppo.ml"
                                     -> List.map free_in ts
                 | 
# 397 "ppx_deriving.cppo.ml"
...
# 434 "ppx_deriving.cppo.ml"
    let name = name.txt in
    
# 436 "ppx_deriving.cppo.ml"
...
# 441 "ppx_deriving.cppo.ml"
    let name = name.txt in
    
# 443 "ppx_deriving.cppo.ml"
...
# 448 "ppx_deriving.cppo.ml"
    let name = name.txt in
    
# 450 "ppx_deriving.cppo.ml"
...
# 455 "ppx_deriving.cppo.ml"
    let name = name.txt in
    
# 457 "ppx_deriving.cppo.ml"
...
# 462 "ppx_deriving.cppo.ml"
    let name = name.txt in
    
# 464 "ppx_deriving.cppo.ml"
...
# 470 "ppx_deriving.cppo.ml"
      Typ.var ~loc:name.loc name.txt
    
# 474 "ppx_deriving.cppo.ml"
    in
...
# 595 "ppx_deriving.cppo.ml"
      String.capitalize_ascii
    
# 599 "ppx_deriving.cppo.ml"
    in
...
# 618 "ppx_deriving.cppo.ml"
    rec_flag
  
# 620 "ppx_deriving.cppo.ml"
  | _ -> assert false
