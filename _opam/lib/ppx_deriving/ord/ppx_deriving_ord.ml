...
# 139 "ppx_deriving_ord.cppo.ml"
        Pat.variant label.txt popt
      
# 141 "ppx_deriving_ord.cppo.ml"
      in
      let cases =
        fields |> List.map (fun field ->
          ...
          match field with
          | 
# 146 "ppx_deriving_ord.cppo.ml"
             {prf_desc = Rtag(label,  true (*empty*),  []); _}  
# 146 "ppx_deriving_ord.cppo.ml"
                                                 ->
            Exp.case (pdup (fun _ -> variant label None)) [%expr 0]
          | 
# 148 "ppx_deriving_ord.cppo.ml"
             {prf_desc = Rtag(label,  false,  [typ]); _}  
# 148 "ppx_deriving_ord.cppo.ml"
                                           ->
            Exp.case (pdup (fun var -> variant label (Some (pvar var))))
                     (app (expr_of_typ typ) [evar "lhs"; evar "rhs"])
          | 
# 151 "ppx_deriving_ord.cppo.ml"
             {prf_desc = Rinherit({ ptyp_desc = Ptyp_constr (tname, _) } as typ); _}  
# 151 "ppx_deriving_ord.cppo.ml"
                                                                         ->
      ...
      let int_cases =
        fields |> List.mapi (fun i field ->
          match field with
          | 
# 161 "ppx_deriving_ord.cppo.ml"
             {prf_desc = Rtag(label,  true (*empty*),  []); _}  
# 161 "ppx_deriving_ord.cppo.ml"
                                                 ->
            Exp.case (variant label None) (int i)
          | 
# 163 "ppx_deriving_ord.cppo.ml"
             {prf_desc = Rtag(label,  false,  [typ]); _}  
# 163 "ppx_deriving_ord.cppo.ml"
                                           ->
            Exp.case (variant label (Some [%pat? _])) (int i)
          | 
# 165 "ppx_deriving_ord.cppo.ml"
             {prf_desc = Rinherit({ ptyp_desc = Ptyp_constr (tname, []) }); _}  
# 165 "ppx_deriving_ord.cppo.ml"
                                                                   ->
